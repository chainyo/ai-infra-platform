...
- Add a **deploy workflow** (manually triggered + tag-based) that applies Layer 1 Terraform and bootstraps a target cluster on Hetzner.
...
- `ci-deploy`: Controlled deployment workflow for applying Layer 1 Terraform and bootstrapping a cluster on Hetzner (manual trigger + semver tags).
