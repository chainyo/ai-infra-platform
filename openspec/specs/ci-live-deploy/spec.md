...
The workflow SHALL connect to the live cluster using a kubeconfig exported from Terraform state and run `argocd app sync --all` (or equivalent), waiting for all Applications to reach `Synced` and `Healthy` status.
...
### Requirement: Live cluster kubeconfig is sourced from Terraform state
The workflow SHALL authenticate to the live cluster using a kubeconfig exported from the production Terraform state. The kubeconfig SHALL never be printed in workflow logs.

#### Scenario: Terraform state contains kubeconfig output
- **WHEN** the live-deploy workflow runs
- **THEN** it SHALL export the `kubeconfig` Terraform output to a temp file, use it for all `kubectl` and `argocd` commands, and delete the temp file at the end of the job

#### Scenario: Terraform state access fails
- **WHEN** the workflow cannot read the production Terraform state or the `kubeconfig` output is unavailable
- **THEN** the workflow SHALL fail immediately with a clear error before running any ArgoCD sync commands
